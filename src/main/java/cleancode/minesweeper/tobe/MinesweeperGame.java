...
public class MinesweeperGame {
    ...
    private static boolean isAllCellOpened() {
        return Arrays.stream(BOARD).toList()
    ...
    private static void initializeGame() {
        ...
        for (int row = 0; row < BOARD_ROW_SIZE; row++) {
            for (int col = 0; col < BOARD_COL_SIZE; col++) {
                countNearbyLandMines(row, col);
            }
        }
    }

    private static void countNearbyLandMines(int row, int col) {
        int count = 0;
        if (isNotLandMineCell(row, col)) {
    ...
    private static boolean isNotLandMineCell(int row, int col) {
